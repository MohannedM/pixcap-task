interface IEmployee {
  ...
  class EmployeeOrgApp implements IEmployeeOrgApp {
    ...
    private findEmployee(param: number | string): IEmployee | null {
      ...
      const found = employees.find((employee) => employee.name === param);
    ...
    move(employeeID: number, supervisorID: number): void {
      ...
      if (employee && supervisor && employee.supervisorId) {
    ...
    undo(): void {
      if (this.undoActions.length) {
        const result = this.undoActions.pop()
        if (result) {
            this.redoActions = result.split("-");
